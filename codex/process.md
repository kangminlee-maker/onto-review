...
| 팀 리뷰 | `processes/review.md` | 8인 패널 리뷰 (멀티에이전트) | 학습 → 승격 |
| 코드 기반 구축 | `processes/build.md` | 코드에서 온톨로지 추출 (멀티에이전트) | → 변환, 리뷰 |
| 변환 | `processes/transform.md` | Raw Ontology 형식 변환 | 구축 → |
| 학습 승격 | `processes/promote.md` | 프로젝트 학습을 글로벌로 승격 (멀티에이전트) | 리뷰/질문 → |
...
경로: `~/.codex/agent-memory/domains/{domain}/`
...
1. **프로젝트 선언 우선**: 프로젝트의 AGENTS.md에 `domain:` 또는 `agent-domain:` 선언이 있으면 해당 도메인을 사용합니다.
...
4. **도메인 문서 없음**: 선언된 도메인의 문서(`~/.codex/agent-memory/domains/{domain}/`)가 존재하지 않으면, 도메인 규칙 없이 범용 원칙만으로 검증합니다.
...
| 학습 유형 | 저장 경로 | 내용 |
|---|---|---|
| **소통 학습 (공통)** | `~/.codex/agent-memory/communication/common.md` | 모든 에이전트에 적용되는 소통 규칙 |
| **소통 학습 (개별)** | `~/.codex/agent-memory/communication/{agent-id}.md` | 특정 에이전트의 소통 선호 |
| **방법론 학습** | `~/.codex/agent-memory/methodology/{agent-id}.md` | 도메인 무관한 검증 원칙, 일반적 교훈 |
| **도메인 학습** | `~/.codex/agent-memory/domains/{domain}/learnings/{agent-id}.md` | 특정 도메인에서만 유효한 학습 |

---

## 멀티에이전트 실행 방식

병렬 에이전트 실행이 필요한 프로세스(팀 리뷰, 코드 기반 구축, 학습 승격)는 **Codex 멀티에이전트**를 우선 사용합니다.

Codex는 `/agent` 명령 또는 자동 판단으로 서브에이전트를 spawn합니다. 각 에이전트는 독립적인 스레드에서 실행되며, 결과를 메인 에이전트(team lead)에게 반환합니다.

### 에이전트 역할 설정

프로젝트 `.codex/config.toml`에서 에이전트 역할을 정의합니다:
...
멀티에이전트 spawn 실패 시, **순차 실행** 방식으로 fallback합니다. 프로세스의 **목적과 출력 형식**은 동일하되, 실행 방식은 다릅니다:
- 각 에이전트의 작업을 메인 컨텍스트에서 순차적으로 실행합니다.
- 각 실행에 에이전트 정의 + 컨텍스트 + 작업 지시를 합쳐서 전달합니다.
- 쟁점 토론(에이전트 간 직접 소통)은 생략합니다. Philosopher가 "쟁점 토론 필요"로 판정하더라도, 미합의 항목은 그대로 최종 보고에 포함합니다.
- Fallback 시 각 실행에 포함해야 하는 내용: 에이전트 정의 + 방법론 학습 + 도메인 문서 + 도메인 학습 + 소통 학습 + 작업 지시.

### 에러 처리 규칙

에러를 2가지로 분류하여 대응합니다:
- **프로세스 중단형**: 리뷰 대상 읽기 실패, 에이전트 정의 파일 읽기 실패 → 프로세스 중단 + 사용자 안내.
- **graceful degradation형**: 에이전트 미응답/실패, 학습 파일 부재, 도메인 문서 부재 → 해당 에이전트를 제외하거나 "아직 없음"으로 처리하고 나머지로 계속 진행. 합의 판정 시 분모를 조정.
...
- 에이전트 간 결과를 교차 공유하지 않는다 (독립성 보장).

**생명주기 관리**: 생성 → 작업 할당 → 에러 처리 → 종료.

### 에이전트 초기 prompt 템플릿

각 에이전트 spawn 시, 정체성 설정 + 자기 로딩 + **Round 1 작업 지시**를 하나의 prompt에 통합합니다. 에이전트는 생성 즉시 작업을 시작하며, 결과를 team lead에게 보고합니다.

team lead는 Context Gathering에서 확보한 **도메인명**, **스킬 경로**, **리뷰 대상**, **시스템 목적**을 resolve하여 변수를 채웁니다.
...
[당신의 정의]
{agents/{agent-id}.md 내용}

[컨텍스트 자기 로딩]
아래 파일들을 직접 읽고 자신의 컨텍스트를 구성하세요. 파일이 없으면 무시하세요:
1. 방법론 학습: ~/.codex/agent-memory/methodology/{agent-id}.md
2. 도메인 문서: ~/.codex/agent-memory/domains/{domain}/{해당 도메인 문서}
3. 도메인 학습 (글로벌): ~/.codex/agent-memory/domains/{domain}/learnings/{agent-id}.md
4. 도메인 학습 (프로젝트): {project}/.codex/learnings/{agent-id}.md
5. 소통 학습 (공통): ~/.codex/agent-memory/communication/common.md
6. 소통 학습 (개별): ~/.codex/agent-memory/communication/{agent-id}.md
...
- team lead가 허용하기 전에는 다른 에이전트에게 직접 메시지를 보내지 마세요.
- 한국어(존댓말)로 답변하세요.
- 비유/은유를 사용하지 마세요.
```

**에이전트 정의**(agents/{agent-id}.md)는 team lead가 읽어서 초기 prompt에 직접 포함합니다 (에이전트당 ~14행, 부담 경미). 나머지 컨텍스트는 에이전트가 자기 로딩합니다.
...
**소통 학습**:
- `~/.codex/agent-memory/communication/common.md` (공통): 모든 에이전트에 적용되는 소통 규칙. 팀 리뷰에서 발견된 사항은 여기에 저장합니다.
- `~/.codex/agent-memory/communication/{agent-id}.md` (개별): 특정 에이전트의 소통 선호. 개별 질문(`$onto-{dimension}`)에서 발견된 사항만 여기에 저장합니다.
...
**방법론 학습** (`~/.codex/agent-memory/methodology/{agent-id}.md`):
...
**도메인 학습**:
- 저장 경로 판별:
  - 프로젝트에 `.codex/learnings/` 디렉토리가 존재하면: `{project}/.codex/learnings/{agent-id}.md` (프로젝트 수준)
  - 존재하지 않으면: `~/.codex/agent-memory/domains/{domain}/learnings/{agent-id}.md` (글로벌 수준)
...
"communication 발견사항이 N건 기록되었습니다. `~/.codex/agent-memory/communication/`에서 확인하시고, 전역 설정(`~/.codex/AGENTS.md`)에 반영할지 결정해 주세요."
