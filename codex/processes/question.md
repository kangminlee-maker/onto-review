...
> 1인 에이전트에게 특정 관점의 질문을 합니다. 멀티에이전트 없이 단일 에이전트로 실행합니다.
...
1. **에이전트 파일 수집**:
   - 정의: `agents/{agent-id}.md` (스킬 설치 경로 기준)
   - 방법론 학습: `~/.codex/agent-memory/methodology/{agent-id}.md`
   - 소통 학습 (공통): `~/.codex/agent-memory/communication/common.md`
   - 소통 학습 (개별): `~/.codex/agent-memory/communication/{agent-id}.md`
   - 파일이 없으면 무시합니다.

2. **도메인 문서 수집**:
   - 도메인 판별 후, `~/.codex/agent-memory/domains/{domain}/` 하위에서 해당 에이전트의 도메인 문서를 읽습니다.

3. **도메인 학습 수집** (반드시 수행):
   - 글로벌: `~/.codex/agent-memory/domains/{domain}/learnings/{agent-id}.md`
   - **프로젝트**: `{project}/.codex/learnings/{agent-id}.md` — 해당 프로젝트에서 축적된 학습. **이 디렉토리가 존재하면 반드시 읽어야 합니다.**
   - 파일이 없으면 무시합니다.

4. **질문 대상 수집**:
   - 질문이 파일/코드를 참조하는 경우: 해당 내용을 읽습니다.
   - 프로젝트의 AGENTS.md, README.md에서 시스템 목적과 원칙을 파악합니다.

### 2. Agent Execution

해당 에이전트를 **1인 실행**합니다. Codex 서브에이전트를 spawn하거나, 메인 컨텍스트에서 직접 실행합니다.
...
[과거 학습 — 방법론]
{~/.codex/agent-memory/methodology/{agent-id}.md 내용. 없으면 "아직 없음"}
