...
**0.1 기존 schema 확인**
- `{project}/.codex/ontology/schema.yml`이 이미 존재하면 내용을 보여주고 재사용 여부를 확인합니다.
- 존재하지 않으면 0.2로 진행합니다.
...
사용자가 선택/커스터마이징한 구조를 `{project}/.codex/ontology/schema.yml`에 저장합니다.
...
Codex 멀티에이전트로 팀(`onto-buildfromcode`)을 구성하고, 7인 reviewer + Philosopher를 에이전트로 spawn합니다.
초기 prompt: `process.md`의 **에이전트 초기 prompt 템플릿** 사용 (팀명: `onto-buildfromcode`).

team lead(구조 조율자)가 7인 reviewer에게 추출 지시를 **개별 전달**합니다.
...
team lead가 7인의 추출 결과를 Philosopher에게 전달합니다.
...
**저장 파일**: `{project}/.codex/ontology/raw.yml`
...
**schema.yml이 없으면** `.codex/ontology/` 디렉토리와 함께 생성합니다.
...
저장 경로:
- Schema: `.codex/ontology/schema.yml`
- Raw Ontology: `.codex/ontology/raw.yml`

### 다음 단계
- `$onto-transform` — 원하는 형식으로 변환
- `$onto-review .codex/ontology/raw.yml` — 구축된 온톨로지를 8인 패널로 검증
```
